...
<Purpose>
Clone a target website from its URL, replicating both visual appearance and core interactive functionality. Uses available browser automation for live page extraction, LLM-driven code generation, and iterative verification with `visual-verdict` for visual scoring.
</Purpose>
...
- Persist web-clone state only in visible project artifacts when it improves continuity.
- Skip external consultation for straightforward extraction; use it only if verification repeatedly fails on the same issue.
</Tool_Usage>

<State_Management>
Persist extraction and progress data only when it improves continuity or reviewability.

Use visible project artifacts such as:

```text
docs/ai/web-clone/<slug>/
```

Recommended files:
- `extraction.json`: `target_url`, `extracted_at`, `screenshot_path`, `landmark_count`, `interactive_count`, `extraction_size_kb`.
- `verdicts.jsonl`: one composite verdict per verification pass.
- `status.json`: final `completed_at` or `failed_at`, residual gaps, and best achieved visual score.

Do not require hidden runtime directories for normal operation.
</State_Management>
...
- [ ] Visual verdict persisted to a visible artifact path when continuity is needed

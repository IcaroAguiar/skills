 ...
 *
 *   review-forge-candidate-state-v1\0
...
function fail(message) {
  process.stderr.write(`review-forge fingerprint: ${message}\n`);
  process.exitCode = 1;
...
function buildIdentity({ repositoryId, mode, base, head, changedFiles, excluded, candidateBinaryFields, excludedBinaryFields = [], snapshots, excludedSnapshots = [] }) {
  const parts = [Buffer.from("review-forge-candidate-state-v1\0", "utf8")];
...
try {
  ...
  if (!process.exitCode) {
    process.stderr.write(`review-forge fingerprint: ${error.message}\n`);
    process.exitCode = 1;
  }
}

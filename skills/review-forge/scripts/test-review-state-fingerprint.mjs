...
const fixtureDirectory = mkdtempSync(join(tmpdir(), "review-forge-fingerprint-"));
...
try {
  git(["init", "--quiet"]);
  git(["config", "user.email", "review-forge-test@example.invalid"]);
  git(["config", "user.name", "Review Forge Test"]);

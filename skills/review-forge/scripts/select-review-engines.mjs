...
function die(message) {
  console.error(`review-forge engine selection failed: ${message}`);
  process.exit(1);
}
...
if (has("--help")) {
  console.log(`review-forge engine selection
...
Registry discovery order:
  --registry, REVIEW_FORGE_ENGINE_REGISTRY, then the protected user
...
const registryCandidates = explicitRegistryPath
  ...
  : [process.env.REVIEW_FORGE_ENGINE_REGISTRY, resolve(homedir(), ".config", "review-forge", "engines.json")].filter(Boolean);
...
function selfTestCandidate(overrides = {}) {
  return {
    ...
    qualification: {
      ...
      evidence: {
        sourceClass: "review-forge-real-diff",
...
function verifyFailClosed() {
  ...
  console.log("review-forge selector self-test passed: incomplete reviewer and fixer registries fail closed");
  process.exit(0);
}

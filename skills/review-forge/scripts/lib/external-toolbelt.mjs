...
const SENSITIVE_ARGUMENT = /(?:access[_-]?token|api[_-]?key|authorization|bearer|cookie|pass(?:word|wd)?|secret|session(?:[_-]?id)?|token)/i;
const URL_VALUE = /https?:\/\/[^\s"'<>]+/gi;
const BEARER_VALUE = /\bBearer\s+[^\s,;}\]]+/gi;
const SENSITIVE_VALUE = /((?:access[_-]?token|api[_-]?key|authorization|bearer|cookie|pass(?:word|wd)?|secret|session(?:[_-]?id)?|token)\s*(?:=|:)\s*)(?:"[^"]*"|'[^']*'|[^\s,;}\]]+)/gi;

// Receipts may be copied to review artifacts. Keep command execution separate from
// what is returned to callers so configured probe URLs and tool output never leak.
export function redactExternalReceiptValue(value) {
  return String(value || "")
...
export function redactExternalReceiptArgs(args) {
...
export function externalToolbelt(repo, shouldRun, allowDownloads = false, selectedToolNames = [], timeoutMs = 60_000) {
  ...
  const tools = [
    ...
    },
    {
      ...
      installHint: "brew install semgrep or uv tool install semgrep",
      runWhen: () => requested.has("semgrep-autofix"),
    },
    {
      name: "trufflehog",
    ...
    },
    {
      ...
      runWhen: () => requested.has("graphql-inspector") && repo.entries.some((entry) => /\.(graphql|gql)$|(^|\/)(graphql|schema)\//i.test(entry.path)),
    },
    {
      name: "buf",
    ...
    },
    {
      ...
      runWhen: () => requested.has("clang-callgraph") && Boolean(firstCppFile),
    },
    {
      name: "osv-scanner",
      ...
      candidates: [
        {
          ...
          env: {
            ...
            GRYPE_DB_CACHE_DIR: "/tmp/review-forge-grype-db",
          },
          downloads: false,
        },
      ],
      ...
      candidates: [
        {
          ...
          env: {
            XDG_CACHE_HOME: "/tmp/review-forge-cache",
          },
          downloads: false,
        },
        {
      ...
      candidates: [
        {
          ...
          args: ["./..."],
          env: {
            GOCACHE: "/tmp/review-forge-go-cache",
            GOMODCACHE: "/tmp/review-forge-go-mod-cache",
          },
          downloads: false,
        },
      ],
    ...
    },
    {
      ...
      candidates: [
        {
          command: "trivy",
          args: ["config", "--quiet", "--skip-check-update", "--cache-dir", "/tmp/review-forge-trivy-cache", repo.root],
          downloads: false,
        },
      ],
    ...
    },
    {
      ...
      runWhen: () => repo.entries.some((entry) => /\.(tsx|jsx)$/.test(entry.path)) && (requested.has("eslint-jsx-a11y") || repo.entries.some((entry) => /(^|\/)(eslint\.config\.[cm]?[jt]s|\.eslintrc(\.|$))/.test(entry.path))),
    },
    {
      name: "axe",
      ...
      runWhen: () => requested.has("axe") && Boolean(a11yTarget),
    },
    {
      name: "size-limit",
      ...
      runWhen: () => requested.has("size-limit") && repo.entries.some((entry) => /(^|\/)(package\.json|vite\.config|webpack\.config|next\.config)/.test(entry.path)),
    },
    {
      ...
      candidates: [
        {
          ...
          args: bundleStatsPath ? [bundleStatsPath, "--mode", "static", "--no-open", "--report", "/tmp/review-forge-bundle-report.html"] : ["--version"],
          downloads: false,
        },
        {
          command: "npx",
          args: bundleStatsPath ? ["--yes", "webpack-bundle-analyzer", bundleStatsPath, "--mode", "static", "--no-open", "--report", "/tmp/review-forge-bundle-report.html"] : ["--yes", "webpack-bundle-analyzer", "--version"],
          downloads: true,
        },
      ],
      installHint: "npm install -D webpack-bundle-analyzer and configure bundleStatsPath",
      runWhen: () => requested.has("webpack-bundle-analyzer") && Boolean(bundleStatsPath),
    },
    {
      name: "autocannon",
      ...
      runWhen: () => requested.has("autocannon") && Boolean(performanceTarget),
    },
    {
      name: "wrk",
      purpose: "HTTP load smoke for configured performance targets",
      ...
      runWhen: () => requested.has("wrk") && Boolean(performanceTarget),
    },
    {
      name: "zap-baseline",
      ...
      candidates: [
        {
          ...
          args: ["-t", dastTarget, "-r", "/tmp/review-forge-zap-report.html"],
          downloads: false,
        },
      ],
      installHint: "Install OWASP ZAP and expose zap-baseline.py",
      runWhen: () => requested.has("zap-baseline") && Boolean(dastTarget),
    },
  ];

  const permitted = new Set(tools.map((tool) => tool.name));
  const selected = new Set([...requested].filter((id) => permitted.has(id)));
  return tools.filter((tool) => (selected.size === 0 || selected.has(tool.name)) && tool.runWhen()).map((tool) => {
    ...
    if (!shouldRun || !candidate) {
      ...
      return { id: tool.name, available, ran: false, status };
    }

...
const fixtureDirectory = mkdtempSync(join(tmpdir(), "review-forge-ratchet-"));
...
function writePacket(packet) {
  writeFileSync(join(gateDirectory, "review-forge-packet.json"), JSON.stringify(packet, null, 2) + "\n");
}
...
try {
  ...
  expect(workflow.includes("review_forge_context.outputs.base"), "generated workflow context outputs not wired to packet/check steps");
  expect(workflow.includes("vendor/collect-review-context.mjs"), "generated workflow does not vendor collector path");
  expect(workflow.includes("Resolve review-forge context"), "generated workflow missing deterministic event-based base resolver");
  ...
  config.policies.reviewForge.calibratedDetectors = [{ rule: "swallowed-error", calibrationEvidence: "review history fixture qg-17" }];
  writeFileSync(configPath, JSON.stringify(config, null, 2) + "\n");
  result = run(join(gateDirectory, "check-ratchet.mjs"));
  expect(!result.ok && result.output.includes("review-forge.blocking"), "an explicitly calibrated deterministic detector did not block");
  ...
  config.policies.reviewForge.calibratedDetectors = [];
  writeFileSync(configPath, JSON.stringify(config, null, 2) + "\n");
  result = run(join(gateDirectory, "check-ratchet.mjs"));
  expect(!result.ok && result.output.includes("review-forge.blocking"), "an adjudicated blocking receipt did not block");

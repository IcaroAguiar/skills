...
const configuredRoot = process.env.REVIEW_FORGE_SMOKE_ROOT;
const root = configuredRoot ? resolve(configuredRoot) : mkdtempSync(join(tmpdir(), "review-forge-smoke-"));
if (configuredRoot) {
  const relativeToTemp = relative(resolve(tmpdir()), root);
  if (!relativeToTemp || relativeToTemp.startsWith("..") || isAbsolute(relativeToTemp)) {
    throw new Error("REVIEW_FORGE_SMOKE_ROOT must stay under the system temporary directory");
  }
  if (existsSync(root)) throw new Error(`REVIEW_FORGE_SMOKE_ROOT already exists: ${root}`);
  mkdirSync(root, { recursive: true });
}
const preserveRoot = process.env.REVIEW_FORGE_SMOKE_PRESERVE === "1";
...
try {
if (process.env.REVIEW_FORGE_SMOKE_FAIL_AFTER_SETUP === "1") {
...
`,
  ...
  "reviewFeedbackPath": "docs/ai/review-forge-feedback.json"
}`,
    files: {
      "docs/ai/review-forge-feedback.json": `{"feedback":[
...
for (const testCase of (process.env.REVIEW_FORGE_SMOKE_FOCUSED === "1" ? [] : cases)) {
...
const receiptSecret = "eyJhbGciOiJIUzI1NiJ9.review-forge-secret";
...
if (process.env.REVIEW_FORGE_SMOKE_FOCUSED !== "1") {
...
expectIncludes("candidate-config-delta", candidateConfigOutput, "Candidate config delta: .review-forge.json (not applied)");
...
const pinnedBaseConfigRepo = createRepo("pinned-base-config", {
  ".review-forge.json": `{"rules":{"magic-string":false}}`,
  "src/status.ts": `export function status() { return "PENDING"; }`,
});
const pinnedBase = commitAll(pinnedBaseConfigRepo, "trusted base policy");
write(join(pinnedBaseConfigRepo, ".review-forge.json"), `{"rules":{"magic-string":true},"a11yTargets":["https://candidate.invalid/dashboard"]}`);
write(join(pinnedBaseConfigRepo, "src/status.ts"), `export function status(input: string) {
  return input === "APPROVED" ? "APPROVED" : "PENDING";
}
`);
const pinnedBaseConfigOutput = collect(pinnedBaseConfigRepo, ["--base", pinnedBase, "--base-config", ".review-forge.json"]);
...
expectIncludes("quality-gate-ratchet", generatedRatchetWorkflow, "name: Review Forge Quality Gate");
expectIncludes("quality-gate-ratchet", generatedRatchetWorkflow, "REVIEW_FORGE_BASE");
expectIncludes("quality-gate-ratchet", generatedRatchetWorkflow, "review-forge-packet.json");
...
write(join(ratchetRepo, "docs/ai/quality-gate/review-forge-packet.json"), `{"repositories":[]}`);
...
console.log("PASS review-forge-namespace");

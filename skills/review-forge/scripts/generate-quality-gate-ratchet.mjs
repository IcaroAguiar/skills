...
function buildConfig(baseline, base, outRelative) {
  return {
    ...
    reportPaths: {
      ...
      reviewForgePacket: `${outRelative}/review-forge-packet.json`,
...
function copyVendorArtifacts(outDirectory) {
  ...
  for (const artifact of artifacts) {
    ...
    if (!existsSync(source)) {
      throw new Error(`Review Forge vendor artifact missing from source tree: ${artifact}`);
    }
...
function changedFiles() {
  const base = process.env.REVIEW_FORGE_BASE || config.base || "origin/main";
...
function reviewForgePacketMetric(feedback) {
  const packet = readJson(config.reportPaths.reviewForgePacket);
  ...
  const configuredDetectors = Array.isArray(config.policies?.reviewForge?.calibratedDetectors) ? config.policies.reviewForge.calibratedDetectors : [];
...
function feedbackMetric() {
  ...
  if (unscopedFalsePositives) warn("review-forge.feedback-unscoped-false-positive", "False-positive feedback without findingId, fingerprint, or finding location is tracked but cannot suppress a future blocker.");
...
const review = reviewForgePacketMetric(feedback);
...
if (config.requiredChecks.reviewForgePacket && review === null) fail("review-forge.missing", "Review Forge packet " + config.reportPaths.reviewForgePacket + " was not found.");
else if (review) {
  if (review.blocking > config.thresholds.maxBlockingReviewFindings) fail("review-forge.blocking", "Review Forge has " + review.blocking + " adjudicated/calibrated blocking finding(s); " + review.signals + " collector signal(s) remain review context only.");
}
...
const rows = [
  ...
  { metric: "Review Forge adjudicated/calibrated blockers", baseline: 0, current: review ? String(review.blocking) + " blockers; " + String(review.signals) + " signals" : "n/a", target: 0, delta: review?.blocking ?? null, status: !failures.some((f) => f.id === "review-forge.blocking" || f.id === "review-forge.missing") },
...
if (eventName === "pull_request") {
  ...
  throw new Error("Unsupported event for Review Forge quality-gate base resolution.");
}
...
function workflowSource(config, outRelative) {
  ...
  const lines = [
    "name: Review Forge Quality Gate",
    "run-name: Review Forge Quality Gate - ${{ github.ref_name }}",
    "",
    "on:",
    "  pull_request:",
    "    branches: [main]",
    ...
    "",
    "concurrency:",
    "  group: review-forge-quality-gate-${{ github.workflow }}-${{ github.ref }}",
  ...
  lines.push(
    ...
    "      - name: Resolve review-forge context",
    "        id: review_forge_context",
    "        env:",
    ...
    ...resolverLines,
    "          NODE",
    "      - name: Review Forge packet",
    "        env:",
    "          REVIEW_FORGE_BASE: ${{ steps.review_forge_context.outputs.base }}",
    "          REVIEW_FORGE_HEAD: ${{ steps.review_forge_context.outputs.head }}",
    `        run: node ${outPrefix}vendor/collect-review-context.mjs --candidate-mode commit --base "${"$"}REVIEW_FORGE_BASE" --head "${"$"}REVIEW_FORGE_HEAD" --json > ${outPrefix}review-forge-packet.json`,
    ...
    `            ${outRelative}/review-forge-packet.json`
  );
  return `${lines.join("\n")}\n`;
}

function readmeSource() {
  return `# Review Forge Quality Gate Ratchet

This package was generated by \`review-forge/scripts/generate-quality-gate-ratchet.mjs\`.
...
function main() {
  ...
  if (args.json) console.log(JSON.stringify(packet, null, 2));
    ...
    console.log(`Review Forge quality gate ratchet ${args.write ? "written" : "planned"} for ${args.root}`);

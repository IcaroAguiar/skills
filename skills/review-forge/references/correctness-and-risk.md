...
`.review-forge.json`, CI definitions, scripts, and URLs. A PR must not be able
...
Load Review Forge policy only from a harness path outside the candidate worktree
(`--config`) or from a declared base revision (`--base` plus
`--base-config`). Read base policy with Git, never from the checked-out
candidate filesystem. If the candidate changes `.review-forge.json` or the
...
Use `templates/review-forge.schema.json` and
`templates/review-forge.example.json` when authoring that protected policy.

...
2. Generate a quality-gate ratchet package:
   ```bash
   node ~/.agents/skills/review-forge/scripts/generate-quality-gate-ratchet.mjs --root . --base origin/main --write
   ```
...
- Review Forge: require the deterministic review packet, but treat its heuristic findings (including high/medium severities) as review signals. Fail only on a blocking finding in an adjudicated receipt with direct evidence, or on a rule explicitly listed in `policies.reviewForge.calibratedDetectors`; leave the generated list empty until the repository has calibrated a deterministic detector.

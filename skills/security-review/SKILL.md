...
Use native security review roles for modern Codex workflows:

- `security-reviewer` for auth, authorization, secrets, privacy, sensitive data, production, billing, fiscal, and critical integrations.
- `mcp-security-reviewer` for MCP servers, connectors, tool schemas, approval modes, scopes, and tool-output trust.
- `prompt-injection-reviewer` when untrusted content can influence prompts or tool calls.
- `test-auditor` to verify that security evidence is deterministic and sufficient.

Keep security review independent from the builder that implemented the change.

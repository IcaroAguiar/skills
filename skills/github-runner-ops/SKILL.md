...
The portable host package is under `assets/runner-host/`. Its image installs the complete pinned Node distribution under `/usr/local`. The image build runs `node`, `npm`, `npx`, `corepack`, and the pinned `pnpm`. Copy the directory as a unit to the Linux host, keep it user-private, and use:

...
admit exactly one structured completion receipt, then pass both its id and the
fresh current candidate SHA-256 to the transition command. A material-code
receipt is valid only when it is one of these two forms:

- `REVIEW_FORGE_APPROVAL`: a current Review Forge evidence receipt has verdict
  `APPROVE`, is bound to the exact candidate fingerprint, and each of
  `CORRECTNESS`, `SIMPLIFICATION`, `SEMANTICS`, `DOCUMENTATION`, and
  `VERIFICATION` has a current gate receipt bound to that same fingerprint.
...
Reject a missing receipt, any non-`APPROVE` Review Forge verdict, a missing gate,
stale evidence, a changed graph version, or a fingerprint mismatch between the
candidate, gate evidence, review/acceptance evidence, and transition argument.
`NOT_APPLICABLE` needs a gate-specific rationale; it does not remove the gate.

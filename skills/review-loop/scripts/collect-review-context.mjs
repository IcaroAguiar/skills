...
function readJsonConfig(path) {
  ...
  try {
    ...
    throw new Error(`Invalid review-loop config at ${path}: ${error.message}`);
  }
}
...
function readProtectedHarnessConfig(path, roots) {
  ...
  if (!existsSync(path)) throw new Error(`Protected Review Loop config does not exist: ${path}`);
...
function readPinnedBaseConfig(root, base, path) {
  ...
  try {
    ...
    throw new Error(`Invalid Review Loop base config ${relativePath} at ${base}: ${error.message}`);
  }
}
...
function candidateConfigDelta(root, entries, baseConfigPath) {
  const configPaths = new Set([".review-loop.json"]);
...
function looksLikeReviewForgeSkill(root) {
  ...
  try {
    return /^name:\s*review-loop\s*$/m.test(readFileSync(skillPath, "utf8"));
  } catch {
    return false;
  }
}
...
function candidateConfigFindings(repo) {
  return (repo.candidateConfigDelta || []).map((entry) => ({
    ...
    file: entry.path,
    line: "-",
    text: `Candidate changes Review Loop configuration (${entry.status}) but candidate-head configuration is not trusted or applied to this review packet.`,
...
function calibrationInsightsForRepo(repo, findings) {
  ...
  for (const [rule, stats] of Object.entries(byRule)) {
    ...
    if (stats.severityMismatch >= 2) {
      suggestions.push(`Rule ${rule} has repeated severity mismatch feedback. Consider a .review-loop.json severities override or generic severity adjustment after calibration.`);
    }
  }
  return {
...
if (repos.length === 0) {
  ...
  console.log("# Review Loop Packet");
  console.log("");

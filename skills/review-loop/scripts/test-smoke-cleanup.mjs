...
const parent = mkdtempSync(join(tmpdir(), "review-loop-smoke-cleanup-"));
...
function run(root, extraEnv = {}) {
  try {
    execFileSync(process.execPath, [smoke], {
      encoding: "utf8",
      env: { ...process.env, REVIEW_LOOP_SMOKE_ROOT: root, ...extraEnv },
...
try {
  ...
  if (run(failureRoot, { REVIEW_LOOP_SMOKE_FAIL_AFTER_SETUP: "1" })) throw new Error("injected smoke failure unexpectedly passed");

...
- Summarize key assumptions that materially affect threat ranking or scope, then ask the user to confirm or correct them using `request_user_input` when available.
- Ask 1–3 targeted questions with `request_user_input` to resolve missing context (service owner and environment, scale/users, deployment model, authn/authz, internet exposure, data sensitivity, multi-tenancy). If structured user input is unavailable, ask one concise plain-text question at a time.

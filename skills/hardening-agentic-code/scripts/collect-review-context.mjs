...
function trackedFileEntries(root) {
  ...
  if (!tracked.ok && args.fullRepository) return filesystemFileEntries(root);
...
function configForRoot(root) {
  ...
  const selfReviewConfig = looksLikeAgenticCodeReviewSkill(root) ? {
    ignorePaths: ["scripts/smoke-review-toolbelt.mjs", "templates/*-test.vitest.ts"],
    ignoreDetectorImplementationFindings: true,
    ignoreSkillPackageValidatorFindings: true,
  } : {};
  return mergeConfig(mergeConfig(startConfig, selfReviewConfig), repoConfig);
}
...
function looksLikeAgenticCodeReviewSkill(root) {
  const skillPath = join(root, "SKILL.md");
  if (!existsSync(skillPath) || !existsSync(join(root, "scripts", "collect-review-context.mjs"))) return false;
  try {
    return /^name:\s*(agentic-code-review|reviewing-agentic-code|hardening-agentic-code)\s*$/m.test(readFileSync(skillPath, "utf8"));
  } catch {
    return false;
  }
}
...
function buildRepos() {
  const roots = args.roots.length > 0 ? args.roots.flatMap((root) => discoverReviewRoots(root, args.discoverDepth)) : discoverReviewRoots(startCwd, args.discoverDepth);
